		...
		_lf = new ListField() {
			protected boolean navigationClick(int status, int time) {
				...
				Thread thread = new Thread() {
					public void run() {	
						boolean flag = true;					
						flag = CallWebService.INSTANCE.PolicyStatusWS(selectedPolicy, Constants.IMEI);
						while (flag == false) {
							flag =CallWebService.INSTANCE.PolicyStatusWS( selectedPolicy, Constants.IMEI);
						}
					}
